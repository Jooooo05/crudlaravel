            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-400 text-white">
                    <tr>
                        ...
                        <th class="py-3 px-4 uppercase font-semibold text-sm">Nama Kelurahan</th>
                        <th class="py-3 px-4 uppercase font-semibold text-sm">Nama Kecamatan</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="text-center">
                        ...
                        <td class="py-3 px-4">{{ $kelurahan->kode_kel }}</td>
                        <td class="py-3 px-4">{{ $kelurahan->nama_kelurahan }}</td>
                        <td class="py-3 px-4">{{ $kelurahan->kecamatan->nama_kecamatan }}</td>
                        <td class="py-3 px-4">
                            <div class="flex items-center justify-center">
                                @if ($kelurahan->active == 1)                                
                        ...
                        <td class="py-3 px-4">
                            <div class="flex justify-center">
                                <a href="{{ route('kelurahan.edit', ['id' => $kelurahan->id]) }}" class="showFormEditData bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mr-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                                        <path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32L19.513 8.2Z" />
                                    </svg>
                                </a>
                                <form action="{{ route('kelurahan.delete', ['id' => $kelurahan->id]) }}" method="POST" onsubmit="return confirm('Yakin akan menghapus data ini?')">
                                    @csrf
                                    @method('delete')
        ...
        <div id="addFormContainer" class="hidden">
            <div class="absolute inset-0 flex justify-center items-center bg-black bg-opacity-50">
                <div class="bg-white w-[600px] p-8 rounded-md">
                    <div class=" mb-6">
                        <form action="{{ route("kelurahan.post") }}" method="POST">
                            @csrf
                            <div class="flex flex-col gap-4">
                                <div>
                                    <label for="kode_kel" class="block text-sm font-medium text-gray-700">Kode Kelurahan:</label>
                                    <input type="text" name="kode_kel" id="kode_kel" class="mt-1 p-2 block w-full shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 rounded-md">
                                </div>
                                <div>
                                    <label for="nama_kelurahan" class="block text-sm font-medium text-gray-700">Nama Kelurahan:</label>
                                    <input type="text" name="nama_kelurahan" id="nama_kelurahan" class="mt-1 p-2 block w-full shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 rounded-md">
                                </div>
                                <div>
